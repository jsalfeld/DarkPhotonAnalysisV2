...
void trimscoutMakeTheCardsLMnew_2018C_OptSelMC(string treepath = "merged.root", const char* outfilename = "./someMC.root", bool isMC=false) {
 
   vector<string> filesToRun;
   vector<bool> isData;
   string dirIn="/eos/cms/store/group/phys_exotica/darkPhoton/jakob/newProd/2018/MCDY2018LowMass/DarkPhoton/crab_20200706_185414/200706_165513/0000/";
   //string dirIn="";
   ...
   TTree* outtree = new TTree("tree", "tree");
    ...
    TH1D* mass_OptIso[20];
    TH1D* mass_OptVtxAbsDist[20];
    TH1D* mass_OptVtxRelDist[20];
    TH1D* mass_OptVtxChi[20];

    for(int a=0; a<20; a++){

      mass_OptIso[a]        = new TH1D(Form("mass_OptIso_%d",a),        Form("mass_OptIso_%d",a),5500,0.,11.);
      mass_OptVtxAbsDist[a]        = new TH1D(Form("mass_OptVtxAbsDist_%d",a),        Form("mass_OptVtxAbsDist_%d",a),5500,0.,11.);
      mass_OptVtxRelDist[a]        = new TH1D(Form("mass_OptVtxRelDist_%d",a),        Form("mass_OptVtxRelDist_%d",a),5500,0.,11.);
      mass_OptVtxChi[a]        = new TH1D(Form("mass_OptVtxChi_%d",a),        Form("mass_OptVtxChi_%d",a),5500,0.,11.);

    }

    TH1F* massforLimitFullNoCut = new TH1F("massforLimitFullNoCut","massforLimitFullNoCut",5500,0.,11.);
    TH1F* massforLimitOpt = new TH1F("massforLimitOpt","massforLimitOpt",5500,0.,11.);    
    ...
    TH2F* twodDRvsMass = new TH2F("twodDRvsMass","twodDRvsMass",100,0.,20.,100,0.,6.5);
   ...
   for(unsigned int f=0; f<filesToRun.size(); f++){
    ...
    //TTreeReaderValue<std::vector<float> >            muonvtxind  (reader, "muonvtxind"    );
  ...
  //TTreeReaderValue<std::vector<int> > 	   motherid (reader, "motherid");
   ...
   if (isMC || !isData[f]) {
	...
	if((*tkiso)[goodmuons[0]]<0.08 && (*tkiso)[goodmuons[1]]<0.08 ) passIsoOpt=true;
...
'L1_DoubleMu0_SQ'
	*/
	...
	if(m1ch*m2ch<0. && m1pt>slidePt1 && m2pt>slidePt2 && maxEta<1.9){ 
	  ...
	  massforLimitFullNoCut->Fill(mass);

	  for(int c=0; c<20;c++){

	    if(isoMu1<(float(c)*0.02) && isoMu2<(float(c)*0.02)) 		mass_OptIso[c]->Fill(mass);
	    if(sqrt( ((*vtxX)[0] - BSx)*((*vtxX)[0] - BSx) + ((*vtxY)[0] - BSy)*((*vtxY)[0] - BSy) )  < (0.02*float(c))) 	mass_OptVtxAbsDist[c]->Fill(mass);
	    if(sqrt( ((*vtxX)[0] - BSx)*((*vtxX)[0] - BSx) + ((*vtxY)[0] - BSy)*((*vtxY)[0] - BSy) )/sqrt( ((*vtxYError)[0]*(*vtxYError)[0]) + ((*vtxXError)[0]*(*vtxXError)[0]))   < (0.4*float(c))) 				      mass_OptVtxRelDist[c]->Fill(mass);
	    if((*vtxChi2)[0]< float(c)*float(c)*0.01 )  			mass_OptVtxChi[c]->Fill(mass);
	  }	
	}	
	
	if(m1ch*m2ch<0. && m1pt>slidePt1 && m2pt>slidePt2 && maxEta<1.9 && (  sqrt( ((*vtxX)[0] - BSx)*((*vtxX)[0] - BSx) + ((*vtxY)[0] - BSy)*((*vtxY)[0] - BSy) )/sqrt( ((*vtxYError)[0]*(*vtxYError)[0]) + ((*vtxXError)[0]*(*vtxXError)[0]))  ) < 1.2  && passIsoOpt) massforLimitOpt->Fill(mass);

	if(m1ch*m2ch<0. && m1pt>3 && m2pt>3 && maxEta<1.9 && (  sqrt( ((*vtxX)[0] - BSx)*((*vtxX)[0] - BSx) + ((*vtxY)[0] - BSy)*((*vtxY)[0] - BSy) )/sqrt( ((*vtxYError)[0]*(*vtxYError)[0]) + ((*vtxXError)[0]*(*vtxXError)[0]))  ) < 1.2) twodDRvsMass->Fill(mass,drmm);
   ...
   forResolutionCMassJPsi->Write();
	...
	for (int l=0;l<20;l++){
  		mass_OptIso[l]->Write();
    		mass_OptVtxAbsDist[l]->Write();
    		mass_OptVtxRelDist[l]->Write();
    		mass_OptVtxChi[l]->Write();	
 	}

     massforLimitFullNoCut->Write();
     massforLimitOpt->Write();
